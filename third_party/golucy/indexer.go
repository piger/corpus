...
func (index *Index) NewIndexWriter() *IndexWriter {
	...
	ixWriter := &IndexWriter{
		Index:       index,
		lucyIndexer: C.LucyIndexerNew(index.Schema.lucySchema, (*C.struct_cfish_Obj)(ixLocation), nil, C.int32_t(flags)),
	}
...
func (ixWriter *IndexWriter) AddDoc(doc Document) {
	...
	for k, v := range doc {
		...
		C.LucyDocStore(lDoc, name, (*C.struct_cfish_Obj)(value))
		C.DECREF((*C.struct_cfish_Obj)(name))
		C.DECREF((*C.struct_cfish_Obj)(value))
	}
	C.LucyIndexerAddDoc(ixWriter.lucyIndexer, lDoc, 1.0) // Is 1.0 a sane default?
	C.DECREF((*C.struct_cfish_Obj)(lDoc))
}
...
func (ixWriter *IndexWriter) Close() {
	...
	if ixWriter.lucyIndexer != nil {
		C.DECREF((*C.struct_cfish_Obj)(ixWriter.lucyIndexer))
