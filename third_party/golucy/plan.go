...
func (schema *Schema) AddField(field *Field) {
	...
	defer C.DECREF((*C.struct_cfish_Obj)(name))

	switch field.IndexType {
	case FullTextType:
		specType = (*C.CFishObj)(fullTextSpecType(field))
	case StringType:
		specType = (*C.CFishObj)(stringSpecType(field))
	default:
		panic("Specified IndexType not supported yet")
	}
	C.LucySchemaSpecField(schema.lucySchema, name, (*C.struct_lucy_FieldType)(specType))
}
...
func (schema *Schema) Close() {
	if schema.lucySchema != nil {
		C.DECREF((*C.struct_cfish_Obj)(schema.lucySchema))
...
func NewAnalyzer(language string, stemTerms bool) *Analyzer {
	lang := cb_newf(language)
	defer C.DECREF((*C.struct_cfish_Obj)(lang))
	...
	if stemTerms {
		...
		analyzer = &Analyzer{Language: language, lucyAnalyzer: (*C.struct_lucy_Analyzer)(C.LucyEasyAnalyzerNew(lang))}
	} else {
		...
		analyzer = &Analyzer{Language: language, lucyAnalyzer: (*C.struct_lucy_Analyzer)(C.LucyPolyAnalyzerNew(lang, analyzers))}
	}
	runtime.SetFinalizer(analyzer, freeAnalyzer)
	return analyzer
}
...
func (analyzer *Analyzer) Close() {
	if analyzer.lucyAnalyzer != nil {
		C.DECREF((*C.struct_cfish_Obj)(analyzer.lucyAnalyzer))
		analyzer.lucyAnalyzer = nil
	}
}

func stringSpecType(field *Field) *C.CFishCharBuf {
	return (*C.CFishCharBuf)(C.LucyStringTypeInitOptions(
		C.LucyStringTypeNew(),
...
func fullTextSpecType(field *Field) *C.CFishCharBuf {
	...
	//
	// and another
	return (*C.CFishCharBuf)(C.LucyFullTextTypeInitOptions(
